...
#Look at a strain in the MAstermix
#List of false positive samples seen in more than 1 replicate of wrong MM in fig above: 
...
#End here to also make figures (crashes the loop unpredictably for me)
#}
...
FigRatio <- ggplot(AlleleRatios2, aes(Barcode1_RA, AllelRatio), message=FALSE) +
  ...
  coord_cartesian(ylim=c(0.01, 100), xlim=c(0.0001, 1), expand = F) + #ylim=c(-0, 12.5)
...
heatmap(MatrixAll2, col = colRamp, distfun = dist, revC = T, #margins = c(8,6)
